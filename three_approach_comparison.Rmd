...
genModel <- function(t, y, params)
{
  with(as.list(c(y, params)), {
    liv  <- sum(y[c("h_u","a_p","a_a","b_p","b_a")])
    r_d <- f_40yr_drate(t)
...
costs <- function(solution, params)
{
  ...
  with(as.list(params), {
    ...
    treatment.cost <- 
      c_a  *alt_ext_simpson(diff(solution[,"a_c"])*solution[2:k,"disc"]) +
      c_bs *alt_ext_simpson(diff(solution[,"b_c"])*solution[2:k,"disc"]) +
      c_bd *alt_ext_simpson(diff(solution[,"b_d"])*solution[2:k,"disc"])
    drug.cost <-
      c_tx *365*alt_ext_simpson(solution[,"a_p"]*solution[,"disc"])*step +
      c_alt*365*alt_ext_simpson(solution[,"a_a"]*solution[,"disc"])*step +
      c_tx *365*alt_ext_simpson(solution[,"b_p"]*solution[,"disc"])*step +
      c_alt*365*alt_ext_simpson(solution[,"b_a"]*solution[,"disc"])*step

    # Total living in model
    life <- rowSums(solution[,c("h_u","a_p","a_a","b_p","b_a")])

    # Total possible life units is integral of discounted time
    pQALY <- alt_ext_simpson(life*solution[,"disc"])*step

    # Temp disutility of A
    disA <- 0
    disB <- 0
    
    disA <- disA + d_a*alt_ext_simpson(solution[,'a_q']*solution[,"disc"])*step
      
    # Permanent disutility for B (integration)
    disB <- d_b*alt_ext_simpson(solution[,'b_p']*solution[,"disc"])*step + 
            d_b*alt_ext_simpson(solution[,'b_a']*solution[,"disc"])*step 
...
generate.params <- function(config, i, scenario, disc_rate = inst_rate(0.03, 1))
{
  ...
  params$r_a   <- unname(inst_rate(getval("vRiskA_",risks), getval("vDurationA_",risks))) # Rate of Indication
  params$r_b   <- unname(inst_rate(getval("vRiskB_",risks), getval("vDurationB_",risks))) # Rate of Event
  params$rr_b  <- getval("vRR_B_",risks) # Relative Risk of Event when on Alternate treatment

  # Costs
  params$c_a   <- getval("A_c_",costs)       # Cost of Indication (A)
  ...
  params$c_t   <- config$global$single_test[1] 
                  
  params$d_a   <- getval("A_",disutilities)         # Disutility of Indication
  params$d_at  <- getval("A_",durations)/365        # Duration of Indication in years.
  params$d_b   <- getval("B_Survive_",disutilities) # Disutility of Event
...
times <- seq(0, 80, by=1/365) # 80 years, daily resolution
...
# Compute alive &  death
none <- cbind(none, rowSums(none[,c("h_u","a_p","a_a","b_p","b_a")]))
none <- cbind(none, 1-none[,15] - none[,"b_d"])
class(none) <- c("deSolve", "matrix")

single <- cbind(single, rowSums(single[,c("h_u","a_p","a_a","b_p","b_a")]))
single <- cbind(single, 1-single[,15] - single[,"b_d"])
