...
genModel <- function(t, y, params)
{
  with(as.list(c(y, params)), {
    liv  <- sum(y[c("h_u","h_t","a_p","a_a","b_p","b_a")])
    r_d <- f_40yr_drate(t)

    list(c(h_u = (-r_a-r_d)*h_u,
      h_t = (-r_a-r_d)*h_t,
      a_p = r_a*(1-p_o*p_g)*h_u+r_a*(1-p_g*p_r)*h_t-r_b*a_p -r_d*a_p,
      a_a = r_a*p_o*p_g*h_u+r_a*p_g*p_r*h_t -r_b*rr_b*a_a -r_d*a_a,
      a_c = r_a*h_u+r_a*h_t,
      a_l = r_d*a_q,
      a_e = r_d*a_q - if(t < d_at) 0 else lagderiv(t-d_at, 6),
      a_q = r_a*h_u+r_a*h_t - if(t < d_at) 0 else lagderiv(t-d_at, 5)*exp(-a_e),
  ...
  with(as.list(params), {
    ...
    for(i in 1:n)
    {
      ...
      treatment.cost <- treatment.cost +
        c_a[i]  *sum(diff(solution[,"a_c"])*solution[2:k,"disc"]) +
        c_bs[i] *sum(diff(solution[,"b_c"])*solution[2:k,"disc"]) +
        c_bd[i] *sum(diff(solution[,"b_d"])*solution[2:k,"disc"])
      drug.cost <- drug.cost + 
        c_tx[i] *365*alt_ext_simpson(solution[,"a_p"]*solution[,"disc"])*step +
        c_alt[i]*365*alt_ext_simpson(solution[,"a_a"]*solution[,"disc"])*step +
        c_tx[i] *365*alt_ext_simpson(solution[,"b_p"]*solution[,"disc"])*step +
        c_alt[i]*365*alt_ext_simpson(solution[,"b_a"]*solution[,"disc"])*step
      
      # Sum fatal b events from all conditions
      b_d  <- b_d + solution[k,"b_d"]
    }
    
    # Total living in model
    life <- rowSums(solution[,c("h_u","h_t","a_p","a_a","b_p","b_a")])
    ...
    for(i in 1:n)
    {
      disA <- disA + d_a[i]*alt_ext_simpson(solution[,'a_q']*solution[,"disc"])*step
      
      # Permanent disutility for B (integration)
      disB <- disB + 
              d_b[i]*alt_ext_simpson(solution[,'b_p']*solution[,"disc"])*step + 
              d_b[i]*alt_ext_simpson(solution[,'b_a']*solution[,"disc"])*step 
    }
