...
run.id.base <- "vogi"
Strategies <- c("None-None","None-Single","None-Panel")
...
for (s in Strategies) {
  ce.files <-
    grep("ce-results", grep(s, list.files(file.path(mainDir, subDir)), value =
                              TRUE), value = TRUE)
  cat(ce.files)
  load.ce.result <- function(x)
  {
    parameter.file <- gsub("ce-results", "parameter-values", x)
    load(file.path(mainDir, subDir, x))
    load(file.path(mainDir, subDir, parameter.file))
    temp <-
      ce.results[[2]] %>% tbl_df() %>% cbind(drawn.parameter.values)
    varying1 <-
      suppressWarnings(temp %>% map_dbl( ~ var(., na.rm = TRUE)))
    varying <- names(varying1[which(varying1 > 0)])
    temp <-
      temp[, varying] %>% data.frame() %>% dplyr::mutate(file = x, iteration =
                                                           row_number())  %>%
      dplyr::select(
        file,
        iteration,
  ...
  ce.results <-
    ce.files %>% map_df( ~ load.ce.result(.x)) %>% mutate(psa_id = row_number()) %>%
    mutate(strategy = s)
...
for (i in seq(Strategies)) {
  if (i == 1)
    ce.results <- get(paste0("ce.results.", Strategies2[i]))
  else
  {
...
ce.results <-
  ce.results %>% mutate(strategy = gsub("-", ".", strategy)) %>% select(-iteration, -file)
varying1 <-
...
for (i in seq(length(Strategies))) ce.results[,paste0("NHB_",Strategies2[i])] <- ce.results %>% select(-dplyr::contains("NHB")) %>% select_if(grepl(Strategies2[i],names(.))) %>% mutate_at(vars(contains("COST")),funs(-1*./lambda)) %>% mutate(NHB=rowSums(.)) %>% pull(NHB)
...
parm<-'risk.vProbabilityOrder_SC_A'
range<-c(0,1)
...
```{r,eval=FALSE}
parm1<-'risk.vProbabilityOrder_SC_A'
parm2<-'risk.vProbabilityOrder_SC_B'
range1<-c(0,1)
range2<-c(0,1)
...
```{r CEAC, fig.width=7, fig.height=6,warning=FALSE,eval=FALSE}
  CEAC(lambda_range,Strategies,Outcomes[,-1])
```
