...
draw.latin.hypercube <- function(tt, PSA.N = 10) {
  ...
  {
    params.full %>% reshape2::melt(id.vars = c("param", "psatype", "type")) %>% mutate(paramtype = gsub(paste0(scenario.ids, collapse =
                                                                                                                 "|"), "", variable)) %>%
      mutate(
        variable = gsub("_psa1|_psa2", "", variable) ,
        paramtype = gsub("^_", "", paramtype)
  ...
  params.as.list <-
    setNames(split(params2, 1:nrow(params2)), params2$parameter) %>% purrr::map( ~
                                                                                   as.list(.x))
  params <- unlist(lapply(params.as.list, function(x)
    x$value))
  
  X <- randomLHS(vN_PSA, length(params))
...
drawn.parameter.values <- unique(scenarios$type) %>% purrr::map(~draw.latin.hypercube(tt=.x,PSA.N=vN_PSA) )
...
for (ss in c("none","reactive-single","reactive-panel","preemptive-panel"))
{
  registerDoParallel(cores = n.cores)
  getDoParWorkers()
  
  d <- foreach(ii = seq(vN_PSA), .combine = rbind) %dopar%
  {
    library(deSolve)
    library(dplyr)
  ...
  temp <-
    d %>% tbl_df() %>% cbind(as.data.frame(drawn.parameter.values))
  varying1 <-
    suppressWarnings(temp %>% map_dbl( ~ var(., na.rm = TRUE)))
  varying <- unique(c("dQALY","dCOST",names(varying1[which(varying1 > 0)])))
  temp <-
