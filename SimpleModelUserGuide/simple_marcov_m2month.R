...
param <- define_parameters(
        ...
        age_init = 40,
        age = age_init + ceiling(markov_cycle/12),
...
dr <- rescale_discount_rate(x=0.03,from=12,to=1) # discounting rate
#states
...
state_ABS <- define_state(
        ...
        QALY = discount(1-disuB-disuA*ifelse(state_time<=12,1,0),dr)
)
...
write.csv(cp,file="~/Desktop/M2mo_count.csv")
write.csv(cv,file="~/Desktop/M2mo_value.csv")