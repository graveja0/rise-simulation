...
icer <- function(dt) #sum_costs table as input
{
        x <- dt %>% arrange(dCOST) %>% mutate(ICER = (lag(dCOST)-dCOST)/(lag(dQALY)-dQALY)) 
...
ceplane <- function(dt) { #icer table as input
        if(any(names(dt)=="ext.dominated")) {
                ic <- dt %>% select(strategy,dQALY,dCOST,ICER,dominated,ext.dominated)
        } else {
                ic <- dt %>% mutate(ext.dominated=NA) %>% select(strategy,dQALY,dCOST,ICER,dominated,ext.dominated)
        }
        ic$line <- 1
        ic$line[ic$dominated==1] <- 0 
        ic$line[ic$ext.dominated==1] <- 0
        ggplot(ic, aes(x=dQALY,y=dCOST)) + geom_point() + 
                geom_line(data=ic[ic$line==1,]) + 
                geom_label_repel(aes(label=strategy),size=5) +
                ggtitle("Cost-effectiveness Plane") +
                labs(x="Average Quality-Adjusted Life Years") +
                labs(y="Average Cost") +
