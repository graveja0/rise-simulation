...
# Markov
lines(0:params$horizon, c(0, cumsum(mar$combined_model$eval_strategy_list$reference$values$A_acc/10))[(0:params$horizon)*params$interval+1], col='blue', lty=3, lwd=2)
...
plot(deq[,'time'], 100*rowSums(deq[,c('a_dp','a_da')]),
     ...
     main="Indication Disutility Occupancy", ylim=c(0, 1.1*max(100*rowSums(deq[,c('a_dp','a_da')]))))
# DES
...
lines(c(0,1,1:params$horizon),     
      c(0,0,mar$combined_model$eval_strategy_list$reference$values$A_du_acc[(1:params$horizon)*params$interval]/10),        col='blue', lty=3, lwd=2)
...
lines(x$time/365, 100*x$count/params$n, col='red', lty=2)
# Markov
lines(0:params$horizon, c(0,mar$combined_model$eval_strategy_list$reference$counts$A/10)[(0:params$horizon)*params$interval+1], col='blue', lty=3, lwd=2)
...
# Markov
lines(c(0,1,1:params$horizon),     
      c(0,0,mar$combined_model$eval_strategy_list$reference$counts$BD[(1:params$horizon)*params$interval]/10),
...
```{r icers}
rbind(des = des_icer(params, genotype=des),
      deq = deq_icer(params, genotype=deq),
      mar = markov_icer(params, solution=mar))
```
